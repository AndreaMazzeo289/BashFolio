...
import jsonData from './../json/projects.json' assert {type: 'json'};
...
async function handleNewCommand()
{
  ...
  console.log("Valid: " + valid + " Command: " + command);
...
function validateCommand(command) 
{
  return command === "help" ||
         ...
         command === "resume" ||
         command === "cls" ||
         command === "adaptive background subtraction" ||
         command === "research";
}
...
function handleCommand(command)
{
  if (command === "help")
  {
    ...
    printCommandsItem("projects");
    printCommandsItem("research");
    printCommandsItem("social");
    printCommandsItem("about me");
    printCommandsItem("resume");
  }
  else if (command === "projects")
  {
    createText("Type the project name to see details.");
    printProjectsItem("Instagram bot");
    printProjectsItem("GAN");
    printProjectsItem("Paper");
    printProjectsItem("FPGA Fault Injector");
    printProjectsItem("Set Associative Cache");
    printProjectsItem("Adaptive Background subtraction");
    printProjectsItem("3D object reconstruction");
    printProjectsItem("Euridit italia");
    printProjectsItem("Lorenzo il magnifico");
    printProjectsItem("Object Tracker");
    printProjectsItem("Nessie 2013");
  }
  else if (command = "research")
  {
    printProjectsItem("Usability-based Cross-Layer Reliability Evaluation of Image Processing Applications");
    printProjectsItem("Approximation-Based Fault Tolerance in Image Processing Applications");
    printProjectsItem("Error Modeling for Image Processing Filters accelerated onto SRAM-based FPGAs");
  }
  else if (command === "adaptive background subtraction") {

    const data = getProjectInfo(command);
    console.log(data);
...
function printCommandsItem(text) 
{
  printListItem(text, "lightblue-text");
}

function printProjectsItem(text) 
{
  printListItem(text, "lightgreen-text");
}
...
function getProjectInfo(project)
{
  ...
  for (let el in jsonData.projects) {
    let obj  = jsonData.projects[el]
    console.log(obj);
    if (obj.key == project) {
...
function showProjectInfo(data)
{
  ...
  const imgSource = "./assets/img/projects/" + data.media[0] + ".png";
